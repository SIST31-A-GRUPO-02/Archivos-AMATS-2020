...
function formularioOk() {
    ...
    var d1 = parseInt(document.getElementById("moneda_nac").value);
    var d2 = parseInt(document.getElementById("porc").value);
    if (d1 == 0) {
        M.toast({ html: 'Ingrese precio del proyecto' })
        return false;
    } else {
        if (d2 == 0) {
            var ok = confirm("Quiere seguir sin porcentaje de ganancia ");
            if (ok == true) {
                return true;
            } else {
                return false;
...
function formularioOk2() {

    var d1 = parseInt(document.getElementById("cantidad").value);
    /* var d2 = parseInt(document.getElementById("porc").value); */
    if (d1 == 0) {
        M.toast({ html: 'La Cantidad No Puede Ser 0' })
        return false;
    } else {
        return true;
    }
...
function mostrar_infoUsuario() {
    ...
    objAjax.open("GET", "info_user.php?var=" + "Activate" + "&" + "dato1=" + id);
    objAjax.send();
...
function mostrar_datos() {
    ...
    if (window.XMLHttpRequest) {
        ...
        var objAjax6 = new XMLHttpRequest();
    }
    ...
    objAjax6.onreadystatechange = function() {
        if (objAjax6.readyState == 4 && objAjax6.status == 200) {
            document.getElementById("cuerpo6").innerHTML = objAjax6.responseText;
        }
    }
    if (valor1 == "" || valor2 == "") {
        ...
        document.getElementById("cuerpo6").style.display = "block";
        ...
        if (valor != "") {
            ...
            objAjax6.open("GET", "incrementos.php?var=" + valor + "&" + "dato1=" + valor1 + "&" + "dato2=" + valor2);
            objAjax6.send();
...
function validar_datos_usuarios() {
    ...
    if (dato1 == "") {
        ...
        objAjax.open("GET", "../controladores/actualizacion_user.php?dato1=" + dato1 + "&" + "dato2=" + dato2 + "&" + "dato3=" + dato3 + "&" + "dato4=" + dato4);
        objAjax.send();
    }
}
...
function redireccionar2() {
    location.href = "../vistas/opc_usuario.php";
}
